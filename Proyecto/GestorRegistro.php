...
class GestorRegistro
{
        ...
        function RegistrarProveedor($Nombre,$Fecha_Nacimiento,$Numero_Telefonico,$Correo,$Contrasenia,$Empresa)
        {
            ...
            if ($conn->query($inst)) {
                ...
                if ($conn->query($inst)) {
                    return $Proveedor->getCodigo_Proveedor();
                }
                else
                {
                    $conn->close();
        ...
        function RegistrarAdministrador($Nombre,$Fecha_Nacimiento,$Numero_Telefonico,$Correo,$Contrasenia)
        {
            include("ConexionBD.php");
            ...
            $Administrador->setNumero_Telefonico($Numero_Telefonico);
            $Administrador->setCorreo($Correo);
            $pass = $Contrasenia;
            $Password_Encript = password_hash($pass, PASSWORD_DEFAULT);
            $Administrador->setContrasenia($Password_Encript);

            $Administrador->setImagen($_FILES['Imagen']['name']);
            $Administrador->GenerarCodigoAdmin();

            $inst = "INSERT INTO Cuenta (Correo,Contrasenia) VALUES ('". $Administrador->getCorreo() ."','" . $Administrador->getContrasenia() . "')";
            if ($conn->query($inst)) {
                $inst = "SELECT ID_Cuenta FROM Cuenta WHERE Correo='" . $Administrador->getCorreo() . "'";
                $res = $conn->query(($inst));
                $res2 = $res->fetch_array();
                $Administrador->setID_Cuenta($res2[0]);
                $inst = "INSERT INTO Administrador (ID_Cuenta,Nombre,Fecha_Nacimiento,Numero_Telefonico,Codigo_Administrador) VALUE ('" . $Administrador->getID_Cuenta() . "','" . $Administrador->getNombre() . "','" . $Administrador->getFecha_Nacimiento() . "','" . $Administrador->getNumero_Telefonico() . "','" . $Administrador->getCodigo_Administrador() . "')";
                if ($conn->query($inst)) {
                    return $Administrador->getCodigo_Administrador();
                } else {
                    return false;
                }
            } else {
